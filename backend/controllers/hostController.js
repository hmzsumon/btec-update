...
//update salary from excel
exports.updateSalaryFromExcel = catchAsyncErrors(async (req, res, next) => {
	const data = loadUser();
...
// find all host
exports.findAllHostFromSql = catchAsyncErrors(async (req, res, next) => {
	try {
		const hosts = await new Promise((resolve, reject) => {
			mysqlDB.query('SELECT * FROM bogo_user', (err, result) => {
				if (err) {
					reject(err);
				}
				resolve(result);
		...
		for (let i = 0; i < hosts.length; i++) {
			...
			if (host.family_id === 0) {
				console.log('Host Id: ', host.id, 'Family Id: ', host.family_id);
				familyHost.push({
					id: host.id,
					family_id: host.family_id,
					name: host.nick_name,
					ticket: host.ticket,
				});
			} else {
		...
		res.status(200).json({
			success: true,
			message: 'Hosts found successfully',
			total: familyHost.length,
			length: hosts.length,
			data: familyHost,
		});
	} catch (err) {
